...
export function convertStringToObjectList(inputString: string, options: Option[]): Option[] {
  if (!inputString)
    return null;
...
export function convertObjectListToString(objectList: Option[]): string {
  ...
  const labels = objectList
    .map((option) => option.label).filter((label) => label !== null);

  return labels.join(', ');
}

export function toInputDateString (data: string, inputFormat?: string, outputFormat?: string) {
  if (!data)
    return ''
...
export function toOutputDateString (data: Date, inputFormat?: string, outputFormat?: string) {
  if (!data)
    return null
  return (dayjs(data, inputFormat).isValid())
